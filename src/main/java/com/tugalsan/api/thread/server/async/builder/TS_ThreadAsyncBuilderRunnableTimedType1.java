...
public class TS_ThreadAsyncBuilderRunnableTimedType1<A> {
    ...
    private TS_ThreadAsyncBuilderRunnableTimedType1(Optional<Duration> max, Optional<TGS_Func_In1<A>> run) {
        this.max = max;
        this.run = run;
    }
    final public Optional<Duration> max;
    final public Optional<TGS_Func_In1<A>> run;
    ...
    public static <A> TS_ThreadAsyncBuilderRunnableTimedType1<A> run(TGS_Func_In1<A> run) {
        return new TS_ThreadAsyncBuilderRunnableTimedType1(Optional.empty(), Optional.of(run));
    }

    public static <A> TS_ThreadAsyncBuilderRunnableTimedType1<A> maxTimedRun(Duration max, TGS_Func_In1<A> run) {
