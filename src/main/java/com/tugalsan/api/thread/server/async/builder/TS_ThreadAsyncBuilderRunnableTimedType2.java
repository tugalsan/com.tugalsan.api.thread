...
public class TS_ThreadAsyncBuilderRunnableTimedType2<B> {
    ...
    private TS_ThreadAsyncBuilderRunnableTimedType2(Optional<Duration> max, Optional<TGS_CallableType2Void<TS_ThreadSyncTrigger, B>> run) {
        this.max = max;
        this.run = run;
    }
    final public Optional<Duration> max;
    final public Optional<TGS_CallableType2Void<TS_ThreadSyncTrigger, B>> run;
    ...
    public static <killTrigger, B> TS_ThreadAsyncBuilderRunnableTimedType2<B> run(TGS_CallableType2Void<killTrigger, B> run) {
        return new TS_ThreadAsyncBuilderRunnableTimedType2(Optional.empty(), Optional.of(run));
    }

    public static <killTrigger, B> TS_ThreadAsyncBuilderRunnableTimedType2<B> maxTimedRun(Duration max, TGS_CallableType2Void<killTrigger, B> run) {
