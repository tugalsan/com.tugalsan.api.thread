...
import com.tugalsan.api.function.client.maythrow.uncheckedexceptions.TGS_FuncMTUCE_In5;
...
public class TS_ThreadSyncLockLimitedRunType5<A, B, C, D, E> {
    ...
    public <A, B, C, D, E> void run(TGS_FuncMTUCE_In5<A, B, C, D, E> run, A inputA, B inputB, C inputC, D inputD, E inputE) {
        runUntil(run, null, inputA, inputB, inputC, inputD, inputE);
    }

    public <A, B, C, D, E> void runUntil(TGS_FuncMTUCE_In5<A, B, C, D, E> run, Duration timeout, A inputA, B inputB, C inputC, D inputD, E inputE) {
        try {
            if (timeout == null) {
                lock.lock();
            } else {
