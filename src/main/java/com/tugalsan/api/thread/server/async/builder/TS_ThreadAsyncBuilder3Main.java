...
public class TS_ThreadAsyncBuilder3Main<T> {
    ...
    public <T> TS_ThreadAsyncBuilder4Fin<T> fin(TGS_Func run) {
        TGS_Func_In1<T> initObj = o -> run.run();
        return new TS_ThreadAsyncBuilder4Fin(killTrigger, name, init, main, TS_ThreadAsyncBuilderRunnableTimedType1.run(initObj));
    }

    public <T> TS_ThreadAsyncBuilder4Fin<T> finTimed(Duration max, TGS_Func run) {
        TGS_Func_In1<T> initObj = o -> run.run();
    ...
    public <T> TS_ThreadAsyncBuilder4Fin<T> fin(TGS_Func_In1<T> initObj) {
        return new TS_ThreadAsyncBuilder4Fin(killTrigger, name, init, main, TS_ThreadAsyncBuilderRunnableTimedType1.run(initObj));
    }

    public <T> TS_ThreadAsyncBuilder4Fin<T> finTimed(Duration max, TGS_Func_In1<T> initObj) {
        return new TS_ThreadAsyncBuilder4Fin(killTrigger, name, init, main, TS_ThreadAsyncBuilderRunnableTimedType1.maxTimedRun(max, initObj));
    }

    @Deprecated//Complicated
    private TS_ThreadAsyncBuilderObject<T> build(Optional<TGS_Func_OutBool_In1<T>> valCycleMain, Optional<Duration> durPeriodCycle) {
    ...
    public TS_ThreadAsyncBuilderObject<T> cycle_mainValidation(TGS_Func_OutBool_In1<T> valCycleMain) {
        return build(
    ...
    public TS_ThreadAsyncBuilderObject<T> cycle_mainValidation_mainPeriod(TGS_Func_OutBool_In1<T> valCycleMain, Duration durPeriodCycle) {
        return build(
                valCycleMain == null ? Optional.empty() : Optional.of(valCycleMain),
                durPeriodCycle == null ? Optional.empty() : Optional.of(durPeriodCycle)
        );
